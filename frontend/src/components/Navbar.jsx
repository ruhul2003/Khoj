      ...
      <div className="max-w-[1600px] mx-auto px-6 lg:px-12">
        <div className="flex items-center justify-between h-20 gap-8">
          ...
          <Link href="/" className="flex items-center gap-3 group shrink-0">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-tr from-indigo-600 to-violet-600 flex items-center justify-center text-white font-extrabold text-2xl shadow-lg shadow-indigo-600/20 group-hover:scale-105 transition-transform">
              K
            </div>
            <div>
              <span className="text-2xl font-bold tracking-tight text-white flex items-center gap-2 font-['Bai_Jamjuree']">
                Khoj <span className="text-indigo-400 text-[11px] font-semibold tracking-wider uppercase px-2 py-0.5 rounded-full bg-indigo-500/10 border border-indigo-500/20">Marketplace</span>
              </span>
          ...
          <form onSubmit={handleSearchSubmit} className="flex-1 max-w-2xl hidden md:block">
            <div className="relative">
              <input
                ...
                className="w-full pl-12 pr-28 py-3 bg-zinc-900/90 border border-zinc-800 rounded-2xl text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-indigo-500/80 focus:ring-1 focus:ring-indigo-500/50 transition-all font-medium"
              />
              <Search className="w-4 h-4 text-zinc-400 absolute left-4 top-4" />
              <button
                type="submit"
                className="absolute right-2 top-2 px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs rounded-xl transition-all tracking-wide"
              >
                Search
              </button>
            </div>
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <Link
              href="/sell"
              className="flex items-center gap-2 bg-white text-zinc-950 hover:bg-zinc-200 font-bold text-xs uppercase tracking-wider px-5 py-3 rounded-xl shadow-lg transition-all transform hover:-translate-y-0.5"
            >
            ...
            {user ? (
              <div className="relative">
                ...
                >
                  {user.avatar ? (
                    <img src={user.avatar} alt={user.name} className="w-9 h-9 rounded-full object-cover border border-indigo-500/40" />
                  ) : (
                ...
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-3 w-60 glass-panel rounded-2xl shadow-2xl py-3 border border-zinc-800 z-50">
                    <div className="px-5 py-2.5 border-b border-zinc-800/80 space-y-1">
                      ...
                      <div className="pt-1 flex items-center gap-1.5 text-[11px] font-semibold text-emerald-400">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden py-4 border-t border-zinc-800 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs font-bold uppercase tracking-wider">
              ...
              <Link href="/sell" onClick={() => setIsMobileMenuOpen(false)} className="p-3 rounded-xl bg-indigo-600 text-center text-white">
                + Sell Item
              </Link>
            </div>
          </div>
